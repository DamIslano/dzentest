...
<template>
	<div class="d-flex align-items-center mb-5">
		...
		<div>Orders / 25</div>
		<Modal :isOpen="isOpen" @open-modal="open" @close-modal="close" :products="Object.keys(selectedProduct).length > 0 ? [selectedProduct] : allProducts" />
	</div>
		...
		<div :style="{ 'min-width': fullMenu ? '' : '30%' }" class="me-3">
			...
			>
				<div class="d-flex align-items-center py-1" :class="{ 'justify-content-between': fullMenu }">
					...
					<div v-if="fullMenu" @click.stop="open" class="me-4 hover-overlay d-flex align-items-center justify-content-center rounded-circle delete-icon w-h-50px"><TrashIcon class="w-20px"/></div>
		...
		<div v-if="!fullMenu" class="w-100 card p-0 pt-4 position-relative">
			...
			<ul class="list-group list-group-flush border-top-0">
				<li class="list-group-item p-0">
					<Products @delete-product="setProduct" type="order" :period="false" :productDescription="false" :cost="false" :licenseData="false" :condition="false"/>
				</li>
			</ul>
		</div>
	</div>
