...
<template>
  <div
    v-if="type === 'product'"
    class="d-flex align-items-center mb-5 min-h-35px"
  >
    ...
    <div
      v-if="productType || productSpecification"
  ...
  <div
    :class="{ 'd-flex flex-column': type === 'order' }"
    class="overflow-auto"
  >
    <div
      ...
      :class="[classes, { 'mb-3 card border': type === 'product', 'border-top': type === 'order' }]"
    >
      <div
        class="d-flex align-items-center py-2"
        :class="{ 'justify-content-between': !classes }"
      >
        <div
          class="me-4 product-status min-w-15px"
          :class="product.status === 'Available' ? 'product-available' : 'product-disabled'"
        />
        <div
          v-if="product.icon"
          class="me-4 min-w-80px"
        >
          <img
        ...
        <div
          ...
          :class="[{ 'flex-grow-1': type === 'order' }, product.status === 'Available' ? 'product-available-text' : 'product-disabled-text']"
        >
          {{ product.status }}
        </div>
        <div
          v-if="period"
          class="me-5 text-uppercase min-w-150px"
        >
        ...
        <div
          ...
          class="me-4 min-w-400px text-decoration-underline"
        >
          {{ product.product_description }}
        </div>
        <div
          v-if="licenseData"
          class="me-4 min-w-250px d-flex"
        >
